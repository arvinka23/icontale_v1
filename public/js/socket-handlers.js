...
export function registerSocketHandlers(socket) {
    ...
    socket.on('lobby-error', ({ message, code, params }) => {
        toastError(code ? t(code, params) : message);
        playError();
    });

    socket.on('lobby-closed', ({ reason, code } = {}) => {
        clearSession();
        toastError(code ? t(code) : reason || t('error.lobbyClosed'));
    ...
    socket.on('server-shutdown', ({ message, code }) => {
        toastError(code ? t(code) : message || t('info.serverShutdown'));
    });
    ...
    socket.on('spectator-joined', ({ roomCode, settings, started, currentRound, totalRounds }) => {
        ...
        state.settings = settings;
        if (started) {
            showSpectatorView(`Spiel l?uft ??? Runde ${currentRound}/${totalRounds}`);
        } else {
    ...
    socket.on('spectator-guess-phase', () => {
        showSpectatorView('Ratephase l?uft...');
    });
    ...
    socket.on('story-error', ({ message, code, params }) => {
        toastError(code ? t(code, params) : message);
        playError();
    });
}
